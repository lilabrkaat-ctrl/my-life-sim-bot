...
function getPopulationStats(player) {
    ...
    return { total, available, sick, unhappy,
...
function updatePopulation(player) {
    ...
    for (let type in player.people.population) {
        ...
        pop.sick += newSick;
        pop.count -= Math.floor(newSick * 0.1);
    }
...
function assignLand(player, landType, count) {
    ...
    const newLand = {
        id: 'land_' + Date.now(), type: landType, count,
        plantedAt: Date.now(),
        harvestAt: Date.now() + land.cycleDays * 24 * 60 * 60 * 1000,
        workers: land.workersNeeded * count, waterUsed: 0
    };
...
function waterLand(player, landId) {
    ...
    return { success: true, message: `💧 زمین آبیاری شد!\n💧 -${waterNeeded} آب\n🌾 برداشت بهتر (+۲۰٪ محصول)` };
}
...
function buildPublicBuilding(player, buildingKey) {
    ...
    if (player.people.buildings.includes(buildingKey)) return { success: false, message: '❌ این ساختمان قبلاً ساخته شده!' };
    ...
    for (let item in building.cost) player.inventory[item] -= building.cost[item];
    ...
    return { success: true, message: `🏗️ ${building.emoji} *${building.name}* ساخته شد!\n📝 ${building.description}\n👥 ${building.workersNeeded} کارگر` };
}

function setTaxRate(player, className, rate) {
    initPeople(player);
    if (!populationTypes[className]) return { success: false, message: '❌ طبقه نامعتبر!' };
    if (rate < 0 || rate > 0.60) return { success: false, message: '❌ نرخ مالیات باید بین ۰٪ تا ۶۰٪ باشه!' };
    populationTypes[className].taxRate = rate;
    const effect = rate > 0.30 ? '😡' : rate > 0.15 ? '😐' : '😊';
    return { success: true, message: `📜 نرخ مالیات ${populationTypes[className].emoji} ${populationTypes[className].name}:\n${effect} ${Math.floor(rate * 100)}٪` };
}
...
function makeDecision(player, decisionType, choice) {
    initPeople(player);
    const decisions = {
        festival: { name: 'برگزاری جشن', emoji: '🎉', cost: { gold: 100, food: 50 }, effect: { happiness: 15, hunger: -5 }, message: 'جشن بزرگی برگزار شد! مردم شاد شدن.' },
        foodAid: { name: 'کمک غذایی', emoji: '🍞', cost: { food: 200 }, effect: { hunger: 20, happiness: 5 }, message: 'غذا بین مردم تقسیم شد.' },
        conscription: { name: 'سربازگیری', emoji: '⚔️', cost: { gold: 50 }, effect: { military: 15, happiness: -10, workers: -20 }, message: '۲۰ نفر به ارتش پیوستن.' },
        taxBreak: { name: 'بخشش مالیات', emoji: '🙏', cost: { gold: -200 }, effect: { happiness: 20, justice: 10 }, message: 'مالیات این ماه بخشیده شد.' }
    };
    ...
    for (let item in decision.cost) {
        if (item === 'gold' && decision.cost[item] < 0) {
            ...
            if ((player.inventory[item] || 0) < decision.cost[item]) return { success: false, message: `❌ ${item} کافی نداری!` };
    ...
    return { success: true, message: `${decision.emoji} *${decision.name}*\n${decision.message}` };
}
...
function handleEventChoice(player, choiceIndex) {
    ...
    if (choice.effect.population) { for (let type in player.people.population) player.people.population[type].count = Math.max(0, player.people.population[type].count + Math.floor(choice.effect.population / 5)); message += `👥 ${choice.effect.population > 0 ? '+' : ''}${choice.effect.population} جمعیت\n`; }
    if (choice.effect.iron) { player.inventory.iron = (player.inventory.iron || 0) + choice.effect.iron; message += `⛏️ +${choice.effect.iron} آهن\n`; }
...
function formatPeople(player) {
    ...
    msg += `📦 *ذخایر:*\n🍞 غذا: ${player.people.storage.food}\n💧 آب: ${player.people.storage.water}\n\n`;

    msg += `🌾 *زمین‌ها:* ${player.people.lands.length} عدد\n`;
    if (player.people.lands.length > 0) {
    ...
    msg += `🏗️ *ساختمان‌ها:* ${player.people.buildings.length} عدد\n`;
    if (player.people.buildings.length > 0) {
        for (let buildingKey of player.people.buildings) {
            const building = buildings[buildingKey];
...
function getPeopleKeyboard(player) {
    ...
    buttons.push([{ text: '💰 جمع‌آوری مالیات', callback_data: 'people_tax' }]);
    buttons.push([{ text: '🌾 مدیریت زمین‌ها', callback_data: 'people_lands' }]);
    buttons.push([{ text: '🏗️ ساخت ساختمان', callback_data: 'people_buildings' }]);
    buttons.push([{ text: '📜 تصمیم‌گیری', callback_data: 'people_decisions' }]);

    if (player.people.currentEvent) {
        const event = player.people.currentEvent;
        buttons.push([{ text: `❗ ${event.emoji} ${event.name}`, callback_data: 'people_event' }]);
    }
...
function getLandKeyboard() {
    ...
    for (let type in landTypes) {
        ...
        buttons.push([{ text: `🌾 ${land.emoji} ${land.name} (${land.cost}👑)`, callback_data: `people_land_buy_${type}` }]);
    }

    buttons.push([{ text: '💧 آبیاری همه زمین‌ها', callback_data: 'people_land_water_all' }]);
    buttons.push([{ text: '🌾 برداشت همه زمین‌ها', callback_data: 'people_land_harvest_all' }]);
...
function getDecisionKeyboard() {
    ...
    buttons.push([{ text: '🎉 برگزاری جشن (۱۰۰👑 + ۵۰🍞)', callback_data: 'people_festival' }]);
    buttons.push([{ text: '🍞 کمک غذایی (۲۰۰🍞)', callback_data: 'people_food' }]);
    buttons.push([{ text: '⚔️ سربازگیری (۵۰👑)', callback_data: 'people_draft' }]);
    buttons.push([{ text: '🙏 بخشش مالیات', callback_data: 'people_taxbreak' }]);
    buttons.push([{ text: '🔙 برگشت', callback_data: 'people_menu' }]);
...
module.exports = {
    populationTypes, landTypes, buildings,
    initPeople, getTotalPopulation, getAvailableWorkers, getPopulationStats,
    updatePopulation, assignLand, waterLand, harvestLand, harvestAllLands,
    buildPublicBuilding, setTaxRate, collectTaxes, makeDecision,
    getRandomEvent, handleEventChoice, formatPeople,
    getPeopleKeyboard, getLandKeyboard, getBuildingKeyboard, getDecisionKeyboard
};