    ...
    bot.on('callback_query', async (query) => {
        ...
        try {
            ...
            if (data.startsWith('empire_assign_')) {
                ...
                await del();
                await send('👶 *کدوم فرزند رو می‌خوای منصوب کنی؟*', { reply_markup: { inline_keyboard: btns } });
                return bot.answerCallbackQuery(query.id);
            }
            ...
            if (data.startsWith('empire_wonder_')) {
                ...
                await del();
                await send(formatEmpire(p), getEmpireKeyboard(p));
                return bot.answerCallbackQuery(query.id, { text: result.success ? '✅ ساخته شد!' : (result.message || 'خطا').replace(/[*_]/g, '').substring(0, 60), show_alert: true });
            }
...
if (data.startsWith('harem_queen_')) {
    ...
    info += `👑 ${queen.rank} | 💕 ${queen.points}\n`;
    info += `😊 ${queen.mood}% | ❤️ ${queen.health}%`;
...
if (data === 'harem_pregnancy_new') {
    ...
    await del();
    await send('👸 *ملکه مورد نظر برای بارداری:*', { reply_markup: { inline_keyboard: btns } });
    return bot.answerCallbackQuery(query.id);
}

if (data.startsWith('harem_pregnancy_select_')) {
    const qId = data.replace('harem_pregnancy_select_', '');
    require('./core').haremState[chatId] = { queenId: qId, action: 'startPregnancy' };
...
if (data.startsWith('orgy_condom_') || data.startsWith('orgy_nocondom_')) {
    ...
    // مرحله ۲: گیف خودارضایی + انتخاب پوزیشن
...
if (data.startsWith('orgy_front_') || data.startsWith('orgy_back_') || data.startsWith('orgy_mouth_')) {
    const parts = data.split('_');
    const pos = parts[1]; // front, back, mouth
    const npcId = parts[2];
    ...
    if (useCondom) {
        resultMsg += `\n🎈 کاندوم -۱ (${p.inventory.condom} عدد)`;
    } else {
        resultMsg += `\n⚠️ بدون کاندوم`;
    }
    ...
    if (image) {
        ...
        await bot.sendMessage(chatId, resultMsg, { parse_mode: 'Markdown', ...getQueenKeyboard(p, queen.id) });
    }
    return bot.answerCallbackQuery(query.id);
}
            // ============ 🏛️ دربار ============
            ...
            if (data === 'people_buildings') {
                ...
                await del();
                await send('🏗️ *ساختمان‌های عمومی:*', getBuildingKeyboard());
                return bot.answerCallbackQuery(query.id);
            }
            ...
            try { await bot.answerCallbackQuery(query.id, { text: '❌ خطا: ' + (e.message || '').substring(0, 60), show_alert: true }); } catch(e2) {}
        }
    });

    // ============ 📝 پیام‌های متنی (state ها) ============
