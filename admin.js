...
function adminCommand(player, command, args) {
    ...
    switch (cmd) {
        ...
        case 'max': case 'maxall':
            ...
            // حیوانات - با try-catch
            try {
                if (!player.pets) player.pets = [];
                const { petTypes } = require('./pet');
                player.pets = [
                    { id: 'admin_1', type: 'wolf_spirit', name: 'روح گرگ', emoji: '👻🐺', level: 50, attackBonus: 30, defenseBonus: 15, hpBonus: 60, rarity: 'legendary', foodCost: 1 },
                    { id: 'admin_2', type: 'dragon_ancient', name: 'اژدهای باستانی', emoji: '🐉💀', level: 50, attackBonus: 50, defenseBonus: 25, hpBonus: 100, rarity: 'legendary', foodCost: 1 }
                ];
            } catch (e) {}
        ...
        case 'pet': case 'addpet':
            try {
                ...
                const newPet = {
                    ...
                    foodCost: petData.foodCost || 3,
                    foundAt: Date.now()
                };
                result = addPet(player, newPet);
            } catch (e) {
                result = { success: false, message: '❌ سیستم حیوانات در دسترس نیست!' };
            }
            break;
        ...
        case 'box': case 'addbox':
            try {
                ...
                result = { success: false, message: '❌ خطا در اضافه کردن صندوق!' };
            }
            break;
        ...
        case 'openbox': case 'unbox':
            try {
                ...
                result = { success: false, message: '❌ سیستم صندوقچه در دسترس نیست!' };
            }
            break;
        ...
        case 'quest': case 'newquest':
            try {
                ...
                result = { success: false, message: '❌ سیستم ماموریت در دسترس نیست!' };
            }
            break;
