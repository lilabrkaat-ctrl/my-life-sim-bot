...
function propose(player, npcId) {
    ...
    if (Math.random() < 0.5) {
        ...
        player.marry = npcId;
        // اصلاح: با دو پارامتر صدا زده میشه
        return { success: true, message: `💍 ${npc?.emoji || ''} ${getMarryDialogue('propose', 'accept')}` };
    } else {
        // اصلاح: با دو پارامتر صدا زده میشه
        return { success: false, message: `💔 ${npc?.emoji || ''} ${getMarryDialogue('propose', 'reject')}` };
    }
}
...
function marry(player, npcId) {
    ...
    // اصلاح: با کلید درست صدا زده میشه
    return { success: true, message: `👰 ${npc?.emoji || ''} ${getMarryDialogue('marry', 'text')}\n\n💍 *مزایای ازدواج:*\n❤️ +۲۰۰ جان\n⚔️ +۵۰ حمله\n🛡️ +۲۰ دفاع` };
}
...
function divorce(player, npcId) {
    ...
    // اصلاح: با کلید درست صدا زده میشه
    return { success: true, message: `💔 ${npc?.emoji || ''} ${getMarryDialogue('divorce', 'text')}\n\n💔 *مزایای ازدواج حذف شد*` };
}
