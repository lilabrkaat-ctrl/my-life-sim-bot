...
// =============================================
// 🔒 کیبورد شیشه‌ای زندان
// =============================================
function getPrisonKeyboard(player) {
    const buttons = [];
    ...
    if (player.prison && player.prison.length > 0) {
        for (let prisoner of player.prison) {
            ...
            const relation = getRelationLevel(points);
            buttons.push([{ 
                text: `${prisoner.emoji} ${prisoner.name} | ${relation.name} | ⏰${prisoner.daysUntilEscape} روز`, 
                callback_data: `prison_select_${prisoner.npcId}` 
            }]);
        }
    }
...
function getPrisonerKeyboard(player, npcId) {
    ...
    // لمس - همیشه هست
    buttons.push([{ text: `🖐️ لمس کن (${actions.touch})`, callback_data: `prison_touch_${npcId}` }]);

    // بوسه - نیاز به ۳ لمس
    if (canKiss) {
        buttons.push([{ text: `💋 ببوس (${actions.kiss})`, callback_data: `prison_kiss_${npcId}` }]);
    }

    // عیاشی - نیاز به ۱۰ لمس و ۱۰ بوسه
    if (canOrgy) {
        buttons.push([{ text: `🔥 عیاشی (${actions.orgy})`, callback_data: `prison_orgy_${npcId}` }]);
    }

    // آزادی
    buttons.push([{ text: '🔓 آزاد کن', callback_data: `prison_release_${npcId}` }]);
    buttons.push([{ text: '🔙 برگشت به زندان', callback_data: 'prison_back' }]);
...
module.exports = {
    ...
    formatPrison, getPrisonKeyboard, getPrisonerKeyboard, getPrisonActions
};