...
const intrigueTypes = {
    poison: {
        name: 'مسمومیت',
        emoji: '🧪',
        description: 'هدف رو با سم از پای دربیار',
        requiredClass: 'mage',
        successChance: 0.40,
        caughtChance: 0.35,
    ...
    coup: {
        name: 'کودتا',
        emoji: '🗡️',
        description: 'قدرت رو به زور بگیر',
    ...
    seduction: {
        name: 'اغواگری',
        emoji: '💋',
        description: 'با ملکه رابطه برقرار کن',
        requiredClass: null,
        successChance: 0.35,
        caughtChance: 0.45,
    ...
    bribery: {
        name: 'رشوه',
        emoji: '💰',
        description: 'به مقامات رشوه بده',
        requiredClass: 'sage',
        successChance: 0.50,
        caughtChance: 0.25,
    ...
    rumor: {
        name: 'شایعه‌پراکنی',
        emoji: '🗣️',
        description: 'اعتبار رقیب رو نابود کن',
        requiredClass: null,
        successChance: 0.45,
        caughtChance: 0.30,
    ...
    sabotage: {
        name: 'خرابکاری',
        emoji: '💣',
        description: 'اموال رقیب رو نابود کن',
...
function getCourtKeyboard(player) {
    ...
    // دکمه‌های دسیسه
    buttons.push(['🐍 انجام دسیسه']);
    
    // مدیریت زندانیان
    if (player.court.prisoners.length > 0) {
        buttons.push(['🔒 مدیریت زندانیان']);
    }
...
function getIntrigueKeyboard(player) {
    ...
    for (let key in intrigueTypes) {
        const intrigue = intrigueTypes[key];
        buttons.push([`🐍 ${intrigue.emoji} ${intrigue.name}`]);
    }
...
function getPrisonerKeyboard(player) {
    ...
    for (let i = 0; i < player.court.prisoners.length; i++) {
        const p = player.court.prisoners[i];
        buttons.push([`💀 اعدام ${p.emoji} ${p.name}`]);
        buttons.push([`🔓 آزاد کن ${p.emoji} ${p.name}`]);
    }
