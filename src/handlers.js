...
function mainMenu() {
    return new InlineKeyboard().text("🔍 جذب", "scout").text("👥 بازیکنا", "players").row().text("📊 آمار", "menu_main").text("⏭️ زمان", "menu_time");
}
function timeMenu() {
    return new InlineKeyboard().text("⏭️ ۱", "next_1").text("⏭️ ۲", "next_2").text("⏭️ ۳", "next_3").row().text("⏭️ ۴", "next_4").text("⏭️ ۳۴", "next_34").row().text("🔙", "menu_main");
}
function playerMenu(i) {
    return new InlineKeyboard().text("🏋️ ارتقا", `up_${i}`).text("🤝 قرارداد", `ct_${i}`).text("💰 فروش", `se_${i}`).row().text("🔄 قرض", `loan_${i}`).text("🔙", "players");
}
function upgradeMenu(i) {
    return new InlineKeyboard().text("⚡۵۰M", `tr_${i}_50`).text("🎯۷۵M", `tr_${i}_75`).row().text("💪۱۵۰M", `tr_${i}_150`).text("📚۳۰۰M", `tr_${i}_300`).row().text("🎯۵۰۰M", `tr_${i}_500`).text("🔙", `pv_${i}`);
}
...
function passWeek(s) {
    s.week++;
    s.players.forEach(p => {
        if (p.contract && p.contract.remaining > 0) { s.money += p.contract.monthly; p.contract.remaining--; if (p.contract.remaining === 0) p.contract = null; }
    });
    ...
    if (d.startsWith("sl_")) {
        ...
        for (let i = 0; i < 3; i++) {
            if (li === 0 && Math.random() < 0.05) {
                f.push({ name: SPECIAL.name, pos: SPECIAL.pos, talent: SPECIAL.talent, ability: SPECIAL.ability, age: 21, city: SPECIAL.city, value: SPECIAL.talent*SPECIAL.ability*2, special: true, contract: null, history: ["🤝 پیوستن ویژه"], goals:0, assists:0, cleans:0, conceded:0, tackles:0, games:0 });
            } else {
                ...
                f.push({ name: nm, pos: POSITIONS[Math.floor(Math.random()*4)], talent: t, ability: a, age: Math.floor(Math.random()*10)+16, city: CITIES[Math.floor(Math.random()*10)], value: t*a*2, special: false, contract: null, history: [`🤝 پیوستن (${s.week})`], goals:0, assists:0, cleans:0, conceded:0, tackles:0, games:0 });
            }
        }
        s.temp = f;
        let tx = `🔍 *${L.name}*\n\n`;
