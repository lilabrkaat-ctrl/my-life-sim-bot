...
function passWeek(s) {
    ...
    if(s.week%6===0) n+=`🏆 مسابقه محلی! ${Math.floor(Math.random()*15)+5}M\n`;
...
async function callback(ctx) {
    ...
    if(d.startsWith("next_")){const w=parseInt(d.split("_")[1]);let ev="";for(let i=0;i<w;i++){const e=passWeek(s);if(e)ev+=e;}if(s.week%3===0){ev+=`📰 خبر\n`+s.players.map(p=>`⚽ ${p.name}: 💰${p.value}M`).join("\n");im=G("newspaper");}if(ev.includes("🚑"))im=G("clinic");if(ev.includes("عادل"))im=G("host");if(ev.includes("🏆"))im=G("trophy");return ctx.editMessageMedia({type:"photo",media:im,caption:`⏭️ ${w} هفته\n${ev}\n${s.sum()}`,parse_mode:"Markdown"},{reply_markup:mainMenu()});}
    
    if(d==="scout"){const kb=new InlineKeyboard();LEAGUES.forEach((l,i)=>{const lk=s.money<l.minBudget?" 🔒":"";kb.text(`${l.name}${lk}`,`sl_${i}`).row();});kb.text("🔙","menu_main");return ctx.editMessageMedia({type:"photo",media:G("scout"),caption:box("🔍 جذب بازیکن",`💰 ${s.money}M`),parse_mode:"Markdown"},{reply_markup:kb});}
