// src/handlers.js - کامل با admin.js جدا
...
function passWeek(s) {
    ...
    let n="";
    if(s.week%5===0){const pz=Math.floor(Math.random()*30)+10;const club=CLUBS[Math.floor(Math.random()*10)];s.tournament={club,prize:pz,active:true};n+=`╭────────────────────╮\n│ 🏆 *تورنمنت هفتگی* │\n╰────────────────────╯\n🏟️ ${club}\n💰 ${pz}M\n✅ بازیکن آزاد بفرست!\n\n`;}
...
async function callback(ctx) {
    ...
    if(d.startsWith("tr_")){
        const p=d.split("_"),i=parseInt(p[1]),c=parseInt(p[2]),pl=s.players[i];
        if(s.money<c){await ctx.answerCallbackQuery("❌ موجودی کافی نیست!");return;}
        s.money-=c;
        if(c===15){pl.ability=Math.min(10,pl.ability+2);pl.talent=Math.min(10,pl.talent+1);}else if(c===50){pl.ability=Math.min(10,pl.ability+3);}else if(c===30){pl.ability=Math.min(10,pl.ability+2);}else{pl.ability=Math.min(10,pl.ability+1);}
        pl.value=c===50?pl.talent*pl.ability*3:pl.talent*pl.ability*2;
        await ctx.answerCallbackQuery("💪 ارتقا انجام شد!");
        r=`✅ ${pl.name} 💪${pl.ability} 💰${pl.value}M`;im=G("training");
    }
