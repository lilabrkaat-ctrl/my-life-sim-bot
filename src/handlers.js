...
function mainMenu() { return new InlineKeyboard().text("🔍 جذب", "scout").text("👥 بازیکنا", "players").row().text("🛒 فروشگاه", "shop_menu").text("⏭️ زمان", "menu_time").row().text("📊 آمار", "menu_main"); }
...
function passWeek(s) {
    ...
    let n=""; if(s.week%4===0) n+=`🎙️ عادل: "بازیکنات خوبن!"\n\n`;
    if(s.week%5===0){const pz=Math.floor(Math.random()*30)+10;n+=`🏆 تورنمنت ${CLUBS[Math.floor(Math.random()*10)]}! 💰${pz}M\n\n`;}
