...
function mainMenu() {
    return new InlineKeyboard()
        ...
        .text("📊 آمار", "menu_main").text("⏭️ زمان", "menu_time");
}
function timeMenu() {
    return new InlineKeyboard()
        .text("⏭️ ۱", "next_1").text("⏭️ ۲", "next_2").text("⏭️ ۳", "next_3").row()
        .text("⏭️ ۴", "next_4").text("⏭️ ۳۴", "next_34").row()
        .text("🔙", "menu_main");
}
function playerMenu(i) {
    return new InlineKeyboard()
        .text("🏋️ ارتقا", `up_${i}`).text("🤝 قرارداد", `ct_${i}`).text("💰 فروش", `se_${i}`).row()
        .text("🔄 قرض", `loan_${i}`).text("🔙", "players");
}
function upgradeMenu(i) {
    return new InlineKeyboard()
        .text("⚡۵۰M", `tr_${i}_50`).text("🎯۷۵M", `tr_${i}_75`).row()
        .text("💪۱۵۰M", `tr_${i}_150`).text("📚۳۰۰M", `tr_${i}_300`).row()
        .text("🎯۵۰۰M", `tr_${i}_500`).text("🔙", `pv_${i}`);
}
...
async function start(ctx) {
    ...
    if (db.has(uid)) {
        return ctx.replyWithPhoto(G("main"), {
            caption: db.get(uid).sum(),
            parse_mode: "Markdown",
...
async function callback(ctx) {
    ...
    if (d === "menu_main") {
        return ctx.editMessageMedia(
            { type: "photo", media: G("main"), caption: s.sum(), parse_mode: "Markdown" },
            { reply_markup: mainMenu() }
        );
    }
    ...
    if (d === "menu_time") {
        return ctx.editMessageMedia(
            { type: "photo", media: G("main"), caption: `⏭️ *گذر زمان*\n📅 هفته ${s.week} | فصل ${s.season}`, parse_mode: "Markdown" },
    ...
    if (d.startsWith("next_")) {
        ...
        return ctx.editMessageMedia(
            { type: "photo", media: im, caption: `⏭️ ${w} هفته\n📅 هفته ${s.week} | فصل ${s.season}${ev}\n\n${s.sum()}`, parse_mode: "Markdown" },
            { reply_markup: mainMenu() }
        );
    }
    ...
    if (d === "scout") {
        ...
        kb.text("🔙", "menu_main");
        return ctx.editMessageMedia(
            { type: "photo", media: G("scout"), caption: `🔍 کدوم لیگ؟\n💰 ۵۰M | ${s.money}M`, parse_mode: "Markdown" },
            { reply_markup: kb }
        );
    }
    ...
    if (d.startsWith("sl_")) {
        ...
        const f = [];
        for (let i = 0; i < 3; i++) {
            if (li === 0 && Math.random() < 0.05) {
                f.push({
                    name: SPECIAL.name, pos: SPECIAL.pos, talent: SPECIAL.talent, ability: SPECIAL.ability,
                    age: 21, city: SPECIAL.city, value: SPECIAL.talent*SPECIAL.ability*2, special: true,
                    contract: null, history: ["🤝 پیوستن ویژه"], goals:0, assists:0, cleans:0, conceded:0, tackles:0, games:0
                });
            } else {
                const nm = FIRST_NAMES[Math.floor(Math.random()*20)] + " " + LAST_NAMES[Math.floor(Math.random()*15)];
                ...
                f.push({
                    name: nm, pos: POSITIONS[Math.floor(Math.random()*4)], talent: t, ability: a,
                    age: Math.floor(Math.random()*10)+16, city: CITIES[Math.floor(Math.random()*10)],
                    value: t*a*2, special: false, contract: null,
                    history: [`🤝 پیوستن (${s.week})`], goals:0, assists:0, cleans:0, conceded:0, tackles:0, games:0
                });
            }
        }
        s.temp = f;
    ...
    if (d === "players") {
        if (!s.players.length) {
            return ctx.editMessageMedia(
                { type: "photo", media: G("player"), caption: "👥 خالی!", parse_mode: "Markdown" },
                { reply_markup: mainMenu() }
            );
        }
        const kb = new InlineKeyboard();
        ...
        kb.text("🔙", "menu_main");
        return ctx.editMessageMedia(
            { type: "photo", media: G("player"), caption: `👥 بازیکنات (${s.players.length})`, parse_mode: "Markdown" },
            { reply_markup: kb }
        );
    }
    ...
    if (d.startsWith("pv_")) {
        ...
        return ctx.editMessageMedia(
            { type: "photo", media: G("player"), caption: card(p), parse_mode: "Markdown" },
            { reply_markup: playerMenu(i) }
        );
    }
    ...
    if (d.startsWith("up_")) {
        const i = parseInt(d.split("_")[1]);
        return ctx.editMessageMedia(
            { type: "photo", media: G("training"), caption: `🏋️ ${s.players[i].name}`, parse_mode: "Markdown" },
            { reply_markup: upgradeMenu(i) }
        );
    }
    ...
    if (d.startsWith("tr_")) {
        ...
        const i = parseInt(parts[1]), c = parseInt(parts[2]);
        const p = s.players[i];
        if (s.money < c) { await ctx.answerCallbackQuery("❌ پول کم!"); return; }
        s.money -= c;
        if (c === 150) { p.ability = Math.min(10, p.ability+2); p.talent = Math.min(10, p.talent+1); }
        else if (c === 500) { p.ability = Math.min(10, p.ability+3); }
        else if (c === 300) { p.ability = Math.min(10, p.ability+2); }
    ...
    if (d.startsWith("ct_")) {
        ...
        const mo = Math.floor(p.value/100);
        const cl = CLUBS[Math.floor(Math.random()*10)];
    ...
    if (d.startsWith("loan_")) {
        ...
        if (p.contract) { await ctx.answerCallbackQuery("❌ قرارداد داره!"); return; }
        const cl = CLUBS[Math.floor(Math.random()*10)];
        const lm = Math.floor(p.value/150);
        s.money += lm;
