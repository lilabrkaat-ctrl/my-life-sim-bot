...
function checkPregnancy(player, npcId, isSpouse, position) {
    ...
    if (Math.random() < chance) {
        const pregnancy = {
            id: 'preg_' + Date.now(), npcId, isSpouse,
            startedAt: now, dueDate: now + pregnancyData.duration * 24 * 60 * 60 * 1000,
    ...
    player.pregnancies = player.pregnancies.filter(pregnancy => {
        if (now >= pregnancy.dueDate) {
            ...
            const child = {
                ...
                motherId: pregnancy.npcId, motherName: pregnancy.motherName,
                motherEmoji: pregnancy.motherEmoji, isSpouse: pregnancy.isSpouse,
                bornAt: now, age: 0, ageStage: 'baby', stageEmoji: '👶',
...
function checkEvolution(player, child) {
    ...
    if (child.xp >= nextLevel.xpNeeded) {
        ...
        if (hasItem || child.isLegendary) {
            ...
            if (nextLevel.skillName !== 'نداره') child.skills.push({ name: nextLevel.skillName, chance: nextLevel.skillChance, level: nextLevel.level });
...
function getChildrenKeyboard(player) {
    ...
    if (player.children && player.children.length > 0) {
        ...
        if (alive.length >= 2) buttons.push([{ text: '⚔️ تورنمنت امپراطوری', callback_data: 'child_tournament' }]);
        const adults = alive.filter(c => c.ageStage === 'adult' || c.ageStage === 'mature' || c.ageStage === 'elder');
        for (let child of adults) {
            if (!child.isHeir) buttons.push([{ text: `👑 ولیعهد کن ${child.emoji} ${child.name}`, callback_data: `child_heir_${child.id}` }]);
        }
    }
...
function handleChildRequest(player, childId, accepted) {
    ...
    if (accepted) {
        if (child.personality === '😇 مهربون') { child.loyalty = Math.min(100, child.loyalty + 15); if (player.people?.stats) player.people.stats.happiness = Math.min(100, (player.people.stats.happiness || 50) + 5); return { success: true, message: `✅ ${child.emoji} *${child.name}* خوشحال شد!\n💕 وفاداری +۱۵\n😊 خوشبختی مردم +۵` }; }
    ...
    } else { child.loyalty = Math.max(0, child.loyalty - 15); return { success: true, message: `❌ ${child.emoji} *${child.name}* ناراحت شد...\n💕 وفاداری -۱۵` }; }
}

module.exports = {
    childImages, childClasses, evolutionPaths, maleNames, femaleNames,
    childPersonalities, pregnancyData,
