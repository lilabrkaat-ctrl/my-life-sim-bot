...
// انواع ماموریت‌ها
const questTypes = {
    hunt_wolf: { name: 'شکار گرگ', description: '۵ تا گرگ شکار کن', emoji: '🐺', target: 'wolf', targetCount: 5, type: 'hunt', rewards: { xp: 30, gold: 20, energy: 10 }, difficulty: 'easy' },
    hunt_snake: { name: 'شکار مار', description: '۴ تا مار شکار کن', emoji: '🐍', target: 'snake', targetCount: 4, type: 'hunt', rewards: { xp: 25, gold: 15, energy: 8 }, difficulty: 'easy' },
    hunt_bear: { name: 'شکار خرس', description: '۳ تا خرس شکار کن', emoji: '🐻', target: 'bear', targetCount: 3, type: 'hunt', rewards: { xp: 50, gold: 40, energy: 15 }, difficulty: 'medium' },
    hunt_dragon: { name: 'شکار اژدها', description: '۱ اژدها شکار کن', emoji: '🐉', target: 'dragon', targetCount: 1, type: 'hunt', rewards: { xp: 100, gold: 80, finisher: 2 }, difficulty: 'hard' },
    gather_wood: { name: 'جمع‌آوری چوب', description: '۱۵ چوب جمع کن', emoji: '🪵', target: 'wood', targetCount: 15, type: 'gather', rewards: { xp: 20, gold: 10, energy: 5 }, difficulty: 'easy' },
    gather_stone: { name: 'جمع‌آوری سنگ', description: '۱۲ سنگ جمع کن', emoji: '🪨', target: 'stone', targetCount: 12, type: 'gather', rewards: { xp: 20, gold: 10, energy: 5 }, difficulty: 'easy' },
    gather_iron: { name: 'جمع‌آوری آهن', description: '۸ آهن جمع کن', emoji: '⛏️', target: 'iron', targetCount: 8, type: 'gather', rewards: { xp: 35, gold: 25, key: 1 }, difficulty: 'medium' },
    travel_forest: { name: 'سفر به جنگل', description: '۳ بار به جنگل سفر کن', emoji: '🌲', target: 'forest', targetCount: 3, type: 'travel', rewards: { xp: 25, gold: 15, energy: 10 }, difficulty: 'easy' },
    travel_mountain: { name: 'سفر به کوهستان', description: '۲ بار به کوهستان سفر کن', emoji: '⛰️', target: 'mountain', targetCount: 2, type: 'travel', rewards: { xp: 30, gold: 20, energy: 12 }, difficulty: 'medium' },
    travel_cave: { name: 'سفر به غار', description: '۲ بار به غار سفر کن', emoji: '🕳️', target: 'cave', targetCount: 2, type: 'travel', rewards: { xp: 40, gold: 30, spell: 1 }, difficulty: 'medium' },
    travel_desert: { name: 'سفر به بیابان', description: '۱ بار به بیابان سفر کن', emoji: '🏜️', target: 'desert', targetCount: 1, type: 'travel', rewards: { xp: 60, gold: 50, wish: 1 }, difficulty: 'hard' },
    defeat_enemies: { name: 'نابودی دشمنان', description: '۱۰ دشمن از هر نوع شکست بده', emoji: '⚔️', target: 'any', targetCount: 10, type: 'defeat', rewards: { xp: 50, gold: 40, finisher: 1 }, difficulty: 'medium' },
    craft_item: { name: 'ساخت‌وساز', description: '۳ آیتم جدید بساز', emoji: '🔨', target: 'any_craft', targetCount: 3, type: 'craft', rewards: { xp: 40, gold: 30, energy: 20 }, difficulty: 'medium' }
};
...
function updateQuestProgress(player, action, target) {
    ...
    for (let quest of player.dailyQuests.quests) {
        ...
        if (questData.type === 'hunt' && action === 'hunt' && target === questData.target) {
            shouldUpdate = true;
        } else if (questData.type === 'gather' && action === 'gather' && target === questData.target) {
            shouldUpdate = true;
        } else if (questData.type === 'travel' && action === 'travel' && target === questData.target) {
            shouldUpdate = true;
        } else if (questData.type === 'defeat' && action === 'defeat') {
            shouldUpdate = true;
        } else if (questData.type === 'craft' && action === 'craft') {
            shouldUpdate = true;
        }
    ...
    if (updated) {
        return checkCompletedQuests(player);
    }
...
function claimQuestReward(player, questKey) {
    if (!player.dailyQuests || !player.dailyQuests.quests) {
        return { success: false, message: '❌ ماموریتی وجود نداره!' };
    }
    ...
    if (leveledUp) {
        rewardMsg += '\n⬆️ لول آپ!';
        if (player.levelUpMessage) {
            rewardMsg += '\n' + player.levelUpMessage;
            player.levelUpMessage = null;
        }
    }
...
function formatDailyQuests(player) {
    ...
    for (let i = 0; i < player.dailyQuests.quests.length; i++) {
        ...
        if (quest.claimed) {
            status = '✅';
            player.dailyQuests.progress[quest.key] = questData.targetCount;
        ...
        msg += `   📝 ${questData.description}\n`;
        msg += `   📊 پیشرفت: ${player.dailyQuests.progress[quest.key] || 0}/${questData.targetCount}\n`;

        if (quest.completed && !quest.claimed) {
            msg += `   🎁 آماده دریافت جایزه!\n`;
        }
    ...
    if (allCompleted) {
        msg += '🎉 *همه ماموریت‌های امروز انجام شد!*\n';
        msg += '🔄 فردا ماموریت‌های جدید...';
    }
