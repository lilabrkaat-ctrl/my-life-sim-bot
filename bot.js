...
bot.on("callback_query:data", async (ctx) => {
    ...
    // ============ منوی اصلی ============
    if (data === "menu_main") {
        return ctx.editMessageText(state.getSummary(), { parse_mode: "Markdown", reply_markup: state.path === "agent" ? agentMenu() : clubMenu() });
    }
    ...
    if (data.startsWith("scout_pick_")) {
        ...
        if (!state.tempPlayers || !state.tempPlayers[index]) {
    ...
    if (data === "agent_players") {
        if (state.players.length === 0) {
            return ctx.editMessageText("👥 هنوز بازیکنی نداری!\nبرو کشف کن.", { parse_mode: "Markdown", reply_markup: agentMenu() });
        }
        ...
        return ctx.editMessageText(`👥 *بازیکنای تو* (${state.players.length} نفر)\n\nیه بازیکن انتخاب کن:`, { parse_mode: "Markdown", reply_markup: kb });
    }
    ...
    if (data.startsWith("player_upgrade_")) {
        ...
        return ctx.editMessageText(`🏋️ *ارتقا ${p.name}*\n⭐${p.talent} 💪${p.ability} | 💰${p.value}M\n\nکدوم تمرین؟`, { parse_mode: "Markdown", reply_markup: kb });
    }
    ...
    if (data === "menu_next") {
        ...
        return ctx.editMessageText(`⏭️ هفته ${state.week} - فصل ${state.season}\n\n${state.getSummary()}`, {
            parse_mode: "Markdown", reply_markup: state.path === "agent" ? agentMenu() : clubMenu()
        });
    }
    ...
    if (data.startsWith("admin_")) {
        ...
        switch(data) {
            ...
            case "admin_scout_star":
                ...
                result = `🌟 ${SPECIAL_PLAYER.name} کشف شد!`;
                break;
        ...
        if (result) return ctx.editMessageText(`🔓 *${result}*\n💰 ${state.money}M | ⭐ ${state.fame}`, { parse_mode: "Markdown", reply_markup: adminMenu() });
    }
