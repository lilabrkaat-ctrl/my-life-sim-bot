...
const { formatCourt, getCourtKeyboard, getIntrigueKeyboard, getCourtEventKeyboard, managePrisoners, getCourtEvent, handleCourtEvent } = require('./court');
...
const { formatSecretChamber, getSecretChamberKeyboard, getChamberRoomKeyboard, getGuardKeyboard, getGiftKeyboard, visitGirl, visitBoy, doActivity, upgradeGirlToQueen, giveGift, hireGuard } = require('./secretChamber');
...
bot.onText(/^⚔️ قدرت/, async (msg) => { const chatId = msg.chat.id; const p = player.getPlayer(chatId); if (!p || !wishState[chatId]) return; p.inventory.wish--; p.attack=(p.attack||5)+20; delete wishState[chatId]; bot.sendMessage(chatId, '⚔️ +۲۰⚔️', {parse_mode:'Markdown',...mainMenu()}); });
bot.onText(/^❤️ سلامتی/, async (msg) => { const chatId = msg.chat.id; const p = player.getPlayer(chatId); if (!p || !wishState[chatId]) return; p.inventory.wish--; p.maxHp=(p.maxHp||100)+100; p.hp=p.maxHp; delete wishState[chatId]; bot.sendMessage(chatId, '❤️ +۱۰۰❤️', {parse_mode:'Markdown',...mainMenu()}); });
bot.onText(/^✨ تجربه/, async (msg) => { const chatId = msg.chat.id; const p = player.getPlayer(chatId); if (!p || !wishState[chatId]) return; p.inventory.wish--; p.xp=(p.xp||0)+100; player.checkLevelUp(p); delete wishState[chatId]; bot.sendMessage(chatId, '✨ +۱۰۰✨', {parse_mode:'Markdown',...mainMenu()}); });
...
bot.onText(/^👩 (.+) \((\d+)👑\)$/, async (msg, match) => {
    ...
    let girl = null;
    for (let g of secretGirls) { if (match[1].includes(g.emoji) && match[1].includes(g.name)) { girl = g; break; } }
    if (!girl) return;
...
bot.onText(/^👦 (.+) \((\d+)👑\)$/, async (msg, match) => {
    ...
    let boy = null;
    for (let b of secretBoys) { if (match[1].includes(b.emoji) && match[1].includes(b.name)) { boy = b; break; } }
    if (!boy) return;
...
bot.onText(/^🔥 از جلو/, async (msg) => {
    ...
    if (hasCondom) {
        await bot.sendMessage(chatId, `${npcEmoji} *${npcName}*: "کاندوم داری؟ 🎈"\n\n🎈 کاندوم: ${p.inventory.condom} عدد`, {
            parse_mode: 'Markdown',
            reply_markup: { keyboard: [['🎈 آره کاندوم دارم'], ['❌ نه نمی‌خوام'], ['🔙 برگشت']], resize_keyboard: true }
        });
        return;
    }
...
bot.onText(/^❌ نه نمی‌خوام/, async (msg) => {
    const chatId = msg.chat.id; const p = player.getPlayer(chatId);
    const npcId = activePrisoner[chatId] || activeHouseNpc[chatId] || activeHaremQueen[chatId];
    if (!p || !npcId) return;
    const isHouse = p.house?.find(h => h.npcId === npcId);
...
bot.onText(/^🎵 آواز$/, async (msg) => { 
    ...
    const dialogue = getPrisonDialogue(npcId, getRelationLevel(getRelationPoints(p, npcId)).level).text; 
    bot.sendMessage(chatId, `🎵 *آواز جادویی!* +۱۵ رابطه\n\n${dialogue}`, { parse_mode: 'Markdown', ...getPrisonerKeyboard(p, npcId) }); 
});
...
bot.onText(/^🧿 اشک$/, async (msg) => { 
    ...
    if (prisoner) { prisoner.daysUntilEscape += 5; bot.sendMessage(chatId, `🧿 *اشک!* +۵ روز\n⏰ ${prisoner.daysUntilEscape} روز`, { parse_mode: 'Markdown', ...getPrisonerKeyboard(p, npcId) }); } 
});
...
bot.onText(/^👸 (.+)$/, async (msg, match) => {
    const chatId = msg.chat.id; const p = player.getPlayer(chatId);
    if (!p || !p.harem || !p.harem.queens) return;
    const queenInfo = match[1].trim();
    const queen = p.harem.queens.find(q => `${q.emoji} ${q.name}` === queenInfo);
    if (!queen) return;
...
bot.onText(/^👗 خرید لباس و جواهر$/, async (msg) => {
    const chatId = msg.chat.id; const p = player.getPlayer(chatId);
    if (!p) return;
    await bot.sendMessage(chatId, '👗 *انتخاب:*', { reply_markup: { keyboard: [['👗 لباس'], ['💍 جواهرات'], ['🔙 برگشت']], resize_keyboard: true } });
...
bot.onText(/^👗 (.+) \((\d+)👑\)$/, async (msg, match) => {
    ...
    if (!dressType) return;
    const queenId = activeHaremQueen[chatId];
    if (!queenId) { delete haremState[chatId]; return; }
